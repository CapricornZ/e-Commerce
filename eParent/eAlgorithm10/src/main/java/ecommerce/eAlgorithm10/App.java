...
public class App {
	...
	public static void main(String[] args) throws Exception {
		...
		while ((lineTxt = bufferedReader.readLine()) != null) {
			...
			boolean toBeRemove = ((ecommerce.base.ISourceRow)sRow).acceptFilter(new FilterVisitor());
			if(toBeRemove){
				logger.info("{}. [跳过]\r\n", number++);
				skip++;
				sRow.print();
		...
		for (int i = 0; i < maxCountOfTaf; i++) {// 每一段的汇总
			...
			logger.info(
					"第{}段 \r\n\t[ SUM:{}, MAX:{}, x:{}({}%), o:{}({}%) ]\r\n",
					i + 1, sum, max, countFalse, (float) countFalse * 100
							/ (float) (countFalse + countTrue), countTrue,
					(float) countTrue * 100 / (float) (countFalse + countTrue));
			logger.info("\t[ ");
			...
			for (int seq = 1; seq <= seqStastic.getMaxCountOfSeq(); seq++) {
				logger.info("\tSEQ {} {x:{}, o:{}}\r\n", seq, seqStastic
						.getCountOfSeqX().get(seq) == null ? 0 : seqStastic
						.getCountOfSeqX().get(seq), seqStastic.getCountOfSeqO()
						.get(seq) == null ? 0 : seqStastic.getCountOfSeqO()
						.get(seq));
			}
