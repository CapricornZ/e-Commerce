...
public class Element6 implements IElement{
	...
	public Element6(char[] source, IExpect pattern){
		this.source = source;
		this.pattern = pattern;
		this.result = new char[]{'_','_','_','_','_','_'};
	}
	@Override
	...
	public String getHtml(int columnIndex) {
		if(this.result[4]==this.result[5] && this.result[4]=='x' && columnIndex>1){
			if(columnIndex<this.source.length)
	...
	public List<Boolean> execute(IElement other, boolean result2this){
		...
		boolean val = expects[0] == (this.source[4]==other.getSource()[4]?Compare.same:Compare.difference);
		rtn.add(val);
		if(result2this)
		...
		if(!rtn.get(0) && this.source.length==6){//若刚才的结果为true,则跳过第二个运算
			val = expects[1] == (this.source[5]==other.getSource()[5]?Compare.same:Compare.difference);
			rtn.add(val);
			if(result2this)
